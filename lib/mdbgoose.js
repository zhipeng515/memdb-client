...
global.MONGOOSE_DRIVER_PATH = path.join(__dirname, 'mongoose-driver'); // '../../../../lib/mongoose-driver'; // path.join(__dirname, 'mongoose-driver');
...
mongoose.connect = function(opts){
    ...
    .then(function(ret){
        ...
        if(opts && opts.backend){
            return P.promisify(connectMongo, mongoose)(opts.backend.url, opts.backend.options);
        }
    });
